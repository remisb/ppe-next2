...
| Catalogue item page at `/catalogue/<id>`, opened by clicking its row: current values, status and why an incomplete item cannot be ordered, the item sets that hold it; managers edit and (de)activate it there | web `router` tests; e2e *Item Catalogue: a row opens the item at its own address*, *phone and tablet: …* |
